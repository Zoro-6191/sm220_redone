...
{	
	...
	else timerElem = newHudElem( self );
...
createServerBar( color, width, height, flashFrac, team, selected )
{
	...
	else barElemFrame = newHudElem( self );
	...
	else barElemFrame setShader( "progress_bar_fg", width, height );
	barElemFrame.hidden = false;

	if ( isDefined( team ) )
		barElemBG = newTeamHudElem( team );
	else barElemBG = newHudElem( self );
...
createBar( color, width, height, flashFrac )
{
	...
	else barElemBG setShader( "progress_bar_bg", width + 0, height + 0 );
	barElemBG.hidden = false;
...
createPrimaryProgressBar()
{
	...
	else bar setPoint("CENTER", undefined, level.primaryProgressBarX, level.primaryProgressBarY);

	return bar;
}
createPrimaryProgressBarText()
{
	text = createFontString( "big", level.primaryProgressBarFontSize );
	if ( level.splitScreen )
		text setPoint("TOP", undefined, level.primaryProgressBarTextX, level.primaryProgressBarTextY);
	else text setPoint("CENTER", undefined, level.primaryProgressBarTextX, level.primaryProgressBarTextY);
...
showPerk( index, perk, ypos )
{
	...
	if ( !isdefined( self.perkicon[ index ] ) )
	{
		...
		else ypos = 0 - (90 + iconsize * (2 - index));
